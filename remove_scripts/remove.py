...
class LaneDetection:
    ...
    def perspective_transform(self, frame, image):
        length_line = 380
        orig_img_coord = np.float32([[200, length_line], [200, length_line], [1000, self.height], [1000, self.height]])
        for x in range(4):
    ...
    def detect(self, image):
        self.frame = image #cv2.resize(frame, (self.width, self.height), interpolation = cv2.INTER_AREA)

        frame = self.preprocessing()

        warped_frame = self.perspective_transform(frame, image)
...
if __name__ == '__main__':
    ...
    frame = cv2.imread("./test_data/test_origin.png")

    frame, warped_frame = lane_detection.detect(frame)
